...
const AllWellness = ({ getProfiles, profile: { profiles, loading } }) => {
  useEffect(() => {
    getProfiles()
  }, [getProfiles])
  return (
    <Fragment>
      <div className='LearnByCategoryContainer'>
        ...
        <h2>
          ...
          &nbsp;Sign up for a <span className='selectedCategory'>Wellness</span> class
            ...
            {profiles.length > 0 ? (
              profiles.map(profile => (
                <GeoCard userId={profile.user._id}
                  event={profile.events.filter(e => e.eventType === 'Wellness')}
                />
              ))
            ) : (
              <p className='sorry'>
